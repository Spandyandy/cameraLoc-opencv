...
def plot3D(tvec, imageNum, loc):
	...
	ax.plot_surface(xQR, yQR, np.array(xQR)*0, rstride=20, cstride=20, facecolors=qr)


	# Location of Camera and its direction
	ax.scatter(x, y, z, marker='o', color='red')
	# print([x,loc[0]-x],[y,loc[1]-y],[z,loc[2]-z])
	# ax.plot3D([x,loc[0]-x],[y,loc[1]-y],zs=z,zdir=loc[2]-z, label='Camera Angle')
